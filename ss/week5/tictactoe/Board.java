...
public class Board {
	...
	public boolean hasRow(Mark m) {
		...
		for (int row = 0; row < DIM; row++) {
			...
			for (int col = 0; col < DIM; col++) {
				if (getField(row * DIM + col).equals(m))
					a++;
			}
			if (a == DIM)
				ans = true;
	...
	public boolean hasColumn(Mark m) {
		...
		for (int col = 0; col < DIM; col++) {
			int a = 0;
			for (int row = 0; row < DIM; row++) {
				if (getField(index(row, col)).equals(m))
					a++;
			}
			if (a == DIM)
				ans = true;
	...
	public boolean hasDiagonal(Mark m) {
		...
		for (int col = 0; col < DIM; col++) {
			if (getField(index(row, col)).equals(m))
				a++;
			if (getField(index(DIM - 1 - row, col)).equals(m))
				b++;
		}
		row++;
		if (a == DIM || b == DIM)
	...
	public boolean isWinner(Mark m) {
		System.out.println(hasColumn(m) + " " + hasDiagonal(m) + " " + hasRow(m));
