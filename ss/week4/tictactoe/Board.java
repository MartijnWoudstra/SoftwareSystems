...
public class Board {
    ...
    public boolean hasRow(Mark m) {
    	...
    	for(int row = 0; row < DIM; row++) {
    		...
    		for(int col = 0; col < DIM; col++) {
    			if(getField(row * DIM + col).equals(m)) a++;
    		}
    		if (a == DIM) ans = true;
    		else a = 0;
    	}
    ...
    public boolean hasColumn(Mark m) {
    	...
    	for(int col = 0; col < DIM; col++) {
    		int a = 0;
    		for(int row = 0; row < DIM; row++) {
    			if(getField(index(row, col)).equals(m)) a++;
    		}
    		if (a == DIM) ans = true;
    		else a = 0;
    	}
    ...
    public boolean hasDiagonal(Mark m) {
    	...
    	for(int row = 0; row < DIM; row++) {
    		for(int col = 0; col < DIM; col++){
    			if(getField(index(row, col)).equals(m)) a++;
    			if(getField(index(DIM-1-row, col)).equals(m)) b++;
    		}
    	}
    	if (a == DIM || b == DIM) ans = true;
    	return ans;
    }
